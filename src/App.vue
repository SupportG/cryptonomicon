<template>
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    <!-- <div v-for="(item, idx) in coinList" :key="idx">
      {{ idx }}
    </div> -->
    <div
      v-if="false"
      ...
      <section>
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div
              v-if="filterCoinList.length && ticker"
              class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap"
            >
              <span
                @click="autoAdd(item.Symbol)"
                v-for="(item, idx) in filterCoinList"
                :key="idx"
...
export default {
  ...
  methods: {
    ...
    subscribeToUpdates(tickerName) {
      setInterval(async () => {
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD&api_key=2e9c422da60938363a40cf9520678d0d20c6bf535a3855cb2229bba1f1aa2fa6`
        );
        const data = await f.json();
